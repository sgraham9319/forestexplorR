...
provide quantitative descriptions of local neighborhoods in mapped forest plots.
The neighborhood of a specific location in a mapped plot is the collection of
trees growing within a specified distance of that location. 
...
for the trees of one or more of the plots included in `mapping`. If `stand` is
...
$$arctan(\frac{dbh}{proximity})$$ In the above, `dbh` is the dbh of the neighbor
tree and `proximity` is the distance from the neighborhood center to the
neighbor tree.
...
The required arguments for `tree_utm()` are: geographic coordinates of the plots
(`stand_locs`), the ESPG code of the coordinate reference system of `stand_locs`
(`original_crs`), and the ESPG code of the UTM zone in which the plots exist
(`utm_crs`).
