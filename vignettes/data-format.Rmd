...
This vignette describes how mapped forest plot datasets should be formatted to
ensure full compatibility with ForestPlot and introduces some data checking
functions to assist with the formatting process.

## Data file types

The ForestPlot package expects raw mapped forest plot data to consist of at 
least two distinct files:

1. Mapping data - coordinate locations of individual trees within plots

2. Tree census data - measurements of individual trees taken during repeated
censuses of the plots
...
To ensure full compatibility with ForestPlot functions, all columns shown in
`mapping` should be present in the mapping dataset with columns named exactly
as shown. Additional columns can also be included without disrupting function
performance and column order is not important. For details on what each column
represents, use `?mapping`. If only a few functions from ForestPlot will be 
applied, some columns in `mapping` may not be required (e.g. the column `tag` is
only required for `stand_map()`) so please read the individual function 
documentation before spending time generating any missing columns.

### Tree census data

The tree census dataset should be a data frame where each row represents the 
measurement data for a single tree during a specific census of the plot. This
means each tree will appear on *x* rows where *x* is the number of censuses in 
which that tree was measured. The built-in dataset `tree` is a cleaned and
...
To ensure full compatibility with ForestPlot functions, all columns shown in
`tree` except for `tag` should be present in the tree census dataset with
columns named exactly as shown. Additional columns can also be included without disrupting function performance and column order is not important. For details
on what each column represents, use `?tree`.
...
The ForestPlot package contains functions to assist with the cleaning process
...
check for x and y coordinates beyond the plot boundary.
...
The second element (`$issue_summary`) is a data frame summarizing the number and percentage of trees in the mapping dataset that have one or more problems and
...
The second element (`$issue_summary`) is a data frame summarizing the number and percentage of trees in the tree census dataset that have one or more problems
