...
size_dist <- function(size_data, stands, bin_size = 10){
  ...
  # Make plot
  if(length(stands) > 1){
    ggplot(data = size_data, aes(dbh)) +
      geom_histogram(aes(fill=stand_id),
                     breaks = seq(0, max(size_data$dbh) + bin_size, bin_size)) +
      labs(y = "# trees", x = "Diameter at breast height (cm)") +
      theme_classic() +
      facet_grid(rows = vars(size_data$stand_id)) +
      theme(legend.position = "none")
  } else {
    ggplot(data = size_data, aes(dbh)) +
      geom_histogram(breaks = seq(0, max(size_data$dbh) + bin_size, bin_size)) +
      labs(title = stands, y = "# trees", x = "Diameter at breast height (cm)") +
      theme_classic()
  }
}
